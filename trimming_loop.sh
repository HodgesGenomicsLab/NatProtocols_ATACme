...
FQDIR=/data/hodges_lab/NPCdiff_ATACme/

# Export read1 files
export FILESR1=($(ls -1 ${FQDIR}/*_R1.fastq.gz))

# Export read2 files
export FILESR2=($(ls -1 ${FQDIR}/*_R2.fastq.gz))

# Set directory where trimmed fastq files will be output
export TRIM_DIR=/data/hodges_lab/NPCdiff_ATACme/fastqs/trimmed_reads

# Set directory where fastqc files will be output
export FASTQC_DIR=/data/hodges_lab/NPCdiff_ATACme/fastqs/trimmed_reads_fastqc
